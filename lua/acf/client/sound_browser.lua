...
function PANEL:Sendmenu(sound) // Open a sending and setup menu on right click on a sound file or on pressing the "Sent To" button.
	if isstring(sound) and sound ~= "" then
		...
		if (self.SoundEmitter) then
			MenuButtonOptions:AddOption("Setup Engine sound", function()
				// Setup the Soundemitter stool with the soundpath.
				LocalPlayer():PrintMessage( HUD_PRINTTALK, "The Engine sound has been setup with the soundpath." )
				RunConsoleCommand("acf_sound", sound)
...
function PANEL:CreateSoundBrowser(path) // Make the sound browser panel.
	...
	self.SoundBrowserPanel.PaintOver = function()
		...
		surface.SetTextColor(Color(0,30,0,255))
...
concommand.Add("+acf_sound_browser_open", OpenSoundBrowser)
concommand.Add("-acf_sound_browser_open", CloseSoundBrowser)
