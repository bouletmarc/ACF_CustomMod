...
function MakeACF_Engine5(Owner, Pos, Angle, Id, Data1, Data2, Data3, Data4, Data5, Data6, Data7, Data8, Data9, Data10, Data11, Data12, Data13)
	...
	Engine5.ModTable = Lookup["modtable"]
		...
		if(tonumber(Data3) >= 1) then
			Engine5.PeakTorque = tonumber(Data3)
			Engine5.PeakTorqueHeld = tonumber(Data3)
		...
		Engine5.RequiresFuel = tonumber(Data12)
		Engine5.EngineType = tostring(Data13)
		--#############
			
		Engine5.PeakTorque2 = tonumber(Data3)
		Engine5.PeakTorque3 = tonumber(Data3)
		Engine5.Idling = tonumber(Data4)
		Engine5.PeakMaxRPM2 = tonumber(Data6)
		Engine5.LimitRPM2 = tonumber(Data7)
		Engine5.FlywheelMass3 = tonumber(Data8)
			
		Engine5.Inertia = Engine5.FlywheelMassValue*(3.1416)^2
	--##################################################################################
	Engine5.PowerFuelExtra = 1
	--------------------
	--#### Temperature MOD
	Engine5.Temp = 0				--Temp at Spawn (Degree)
	Engine5.TempMax = Lookup["tempmax"] or 88	--Normal Temp (Degree)
	Engine5.TempWarm = 112			--Warm Temp, Engine take dommage if more than it (Degree)
	Engine5.TempBlow = 125			--Auto Blow Temp, too high (Degree)
	Engine5.EngineHealth = Lookup["enginehealth"] or 100	--Engine Health at Spawn
	Engine5.Blowed = 0
	--Getting Rpm High Temp
	Engine5.TempRpmHighPercent = Engine5.LimitRPM * 0.9 --Getting 90% of the Rpm Band
	if Engine5.TempRpmHighPercent <= Engine5.PeakMaxRPM then
		Engine5.TempRpmHigh = Engine5.PeakMaxRPM
	else
		Engine5.TempRpmHigh = Engine5.LimitRPM * 0.9
	end
	--#####################
	...
	Engine5:SetNetworkedBeamInt("Temp",Engine5.Temp)
	Engine5:SetNetworkedBeamInt("Health",Engine5.EngineHealth)
	Wire_TriggerOutput(Engine5.Entity, "Temperature", Engine5.Temp)
	Wire_TriggerOutput(Engine5.Entity, "Health", Engine5.EngineHealth)
	--####################################################
...
function ENT:Update( ArgsTable )	--That table is the player data, as sorted in the ACFCvars above, with player who shot, and pos and angle of the tool trace inserted at the start
	...
	self.PeakTorque2 = tonumber(ArgsTable[7])
	self.PeakTorque3 = tonumber(ArgsTable[7])
	self.Idling = tonumber(ArgsTable[8])
	self.PeakMaxRPM2 = tonumber(ArgsTable[10])
	self.LimitRPM2 = tonumber(ArgsTable[11])
	self.FlywheelMass3 = tonumber(ArgsTable[12])
