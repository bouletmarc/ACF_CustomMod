...
function MakeACF_Engine2(Owner, Pos, Angle, Id, Data1, Data2, Data3, Data4, Data5, Data6)
	...
	Engine2.ModTable = Lookup["modtable"]
		...
		Engine2.PeakTorque = tonumber(Data1)
		Engine2.PeakTorqueHeld = tonumber(Data1)
		Engine2.IdleRPM = tonumber(Data2)
		Engine2.PeakMinRPM = tonumber(Data3)
		Engine2.LimitRPM = tonumber(Data5)
		if(Data4 <= Data5 ) then 
			Engine2.PeakMaxRPM = tonumber(Data4)
		elseif(Data4 > Data5 ) then
			Engine2.PeakMaxRPM = tonumber(Data5)
		end
		Engine2.FlywheelMassValue = tonumber(Data6)
		Engine2.CutValue = Engine2.LimitRPM / 40
		Engine2.CutRpm = Engine2.LimitRPM - 100
			
		Engine2.PeakTorque2 = tonumber(Data1)
		Engine2.PeakTorque3 = tonumber(Data1)
		Engine2.Idling = tonumber(Data2)
		Engine2.PeakMaxRPM2 = tonumber(Data4)
		Engine2.LimitRPM2 = tonumber(Data5)
		Engine2.FlywheelMass3 = tonumber(Data6)
			
		Engine2.Inertia = Engine2.FlywheelMassValue*(3.1416)^2
	--##################################################################################
	Engine2.PowerFuelExtra = 1
	--------------------
	--#### Temperature MOD
	Engine2.Temp = 0				--Temp at Spawn (Degree)
	Engine2.TempMax = Lookup["tempmax"] or 88	--Normal Temp (Degree)
	Engine2.TempWarm = 112			--Warm Temp, Engine take dommage if more than it (Degree)
	Engine2.TempBlow = 125			--Auto Blow Temp, too high (Degree)
	Engine2.EngineHealth = Lookup["enginehealth"] or 100	--Engine Health at Spawn
	Engine2.Blowed = 0
	--Getting Rpm High Temp
	Engine2.TempRpmHighPercent = Engine2.LimitRPM * 0.9 --Getting 90% of the Rpm Band
	if Engine2.TempRpmHighPercent <= Engine2.PeakMaxRPM then
		Engine2.TempRpmHigh = Engine2.PeakMaxRPM
	else
		Engine2.TempRpmHigh = Engine2.LimitRPM * 0.9
	end
	--#####################
	...
	Engine2:SetNetworkedBeamInt("Temp",Engine2.Temp)
	Engine2:SetNetworkedBeamInt("Health",Engine2.EngineHealth)
	Wire_TriggerOutput(Engine2.Entity, "Temperature", Engine2.Temp)
	Wire_TriggerOutput(Engine2.Entity, "Health", Engine2.EngineHealth)
	--####################################################
